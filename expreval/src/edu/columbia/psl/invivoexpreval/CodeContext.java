...
//                System.out.println("slot: " + slot + ", typeSlot: " + classSlot + ", varSlot: " + varNameSlot);
        ...
        for (int i = 0; i < stackSizes.length; ++i) {
            ...
            if (ss == CodeContext.UNEXAMINED) {
                if (CodeContext.DEBUG) {
                    ...
                    throw new InVivoRuntimeException(functionName + ": Unexamined code at offset " + i);
                }
            }
    ...
    private void flowAnalysis(
        ...
        for (;;) {
            ...
            if (offset < 0 || offset >= codeSize) {
                throw new InVivoRuntimeException(functionName + ": Offset out of range");
            }

            // Have we hit an area that has already been analyzed?
            int css = stackSizes[offset];
            if (css == stackSize) return; // OK.
            if (css == CodeContext.INVALID_OFFSET) throw new InVivoRuntimeException(functionName + ": Invalid offset");
            ...
            default:
                throw new InVivoRuntimeException(functionName + ": Invalid stack delta");
            }
            ...
            if (stackSize < 0) {
                ...
                if (CodeContext.DEBUG) {
                    ...
                    throw new InVivoRuntimeException(msg);
                }
            }
            ...
            default:
                throw new InVivoRuntimeException(functionName + ": Invalid OP1");
            }
            ...
            default:
                throw new InVivoRuntimeException(functionName + ": Invalid OP2");
            }
            ...
            default:
                throw new InVivoRuntimeException(functionName + ": Invalid OP3");
            }
    ...
    private int determineArgumentsSize(short idx) {
        ...
        if (desc.charAt(0) != '(') throw new InVivoRuntimeException("Method descriptor does not start with \"(\"");
        int i = 1;
        int res = 0;
        for (;;) {
            ...
            case '[':
                ...
                if (desc.charAt(i) != 'L') throw new InVivoRuntimeException("Invalid char after \"[\"");
                ++i;
                while (desc.charAt(i++) != ';');
                break;
            case 'L':
                ...
                break;
            default:
                throw new InVivoRuntimeException("Invalid method descriptor");
            }
        }
    }
    ...
    private class Branch extends Relocatable {
        ...
        public boolean relocate() {
            if (this.destination.offset == Offset.UNSET) {
                throw new InVivoRuntimeException("Cannot relocate branch to unset destination offset");
            }
    ...
    private class OffsetBranch extends Relocatable {
        ...
        public boolean relocate() {
            if (this.source.offset == Offset.UNSET || this.destination.offset == Offset.UNSET) {
                throw new InVivoRuntimeException("Cannot relocate offset branch to unset destination offset");
            }
            int offset = this.destination.offset - this.source.offset;
            byte[] ba = new byte[] {
                (byte) (offset >> 24),
    ...
    public void pushInserter(Inserter ins) {
        if (ins.nextInserter != null) throw new InVivoRuntimeException("An Inserter can only be pushed once at a time");
    ...
    public void popInserter() {
        ...
        if (ni == null) throw new InVivoRuntimeException("Code inserter stack underflow");
    ...
    public class Offset {
         ...
         */
        public void set() {
            if (this.offset != Offset.UNSET) throw new InVivoRuntimeException("Cannot \"set()\" Offset more than once");
