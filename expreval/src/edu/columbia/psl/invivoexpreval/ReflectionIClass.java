...
/**
 * Wraps a {@link java.lang.Class} in an {@link edu.columbia.psl.invivoexpreval.IClass}.
 */
...
//            if ((m.getModifiers() & Mod.SYNTHETIC) != 0) continue;
    ...
    private IClass classToIClass(Class c) {
        ...
        try {
            ...
            throw new InVivoRuntimeException("Loading IClass \"" + c.getName() + "\": " + ex);
        }
        if (iClass == null) {
            throw new InVivoRuntimeException("Cannot load class \"" + c.getName() + "\" through the given ClassLoader");
        }
        return iClass;
    }
