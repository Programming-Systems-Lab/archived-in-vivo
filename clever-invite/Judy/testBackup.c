...
float paramEncoding(float arr[], int length){
   ...
   for(j=1;j<length;j++){
      if(j == 1){
	cantVal = bijectiveMapping(arr[0], arr[1]);
      }else{
         cantVal = bijectiveMapping(cantVal,arr[j]);
      }
   }
   return cantVal; 
}
...
*/ 
  ...
  // see if those values have already been seen
	if(judyMember(cantVal, &PLFArray) == -1){ 
		/*If not found in array then add to the corresponding array */ 
		  if(judyInsert(cantVal, &PLFArray) == -1){ 
...
// see if those values have already been seen
	if(judyMember(cantVal,&PLGArray) == -1){ 
		/*If not found in array then add to the corresponding array */ 
		  if(judyInsert(cantVal, &PLGArray) == -1){ 
		    printf("Judy Insertion Error"); 
		    return ERR; 
		  } 
		return 1; 
