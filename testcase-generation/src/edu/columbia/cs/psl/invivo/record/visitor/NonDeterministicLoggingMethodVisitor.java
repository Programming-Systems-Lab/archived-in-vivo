...
public class NonDeterministicLoggingMethodVisitor extends CloningAdviceAdapter implements Constants {
	...
	// dup();
	// super.visitFieldInsn(GETFIELD, owner, BEEN_CLONED_PREFIX+name,
	...
	// super.visitFieldInsn(Opcodes.PUTFIELD, owner,
	// Constants.PREV_VALUE_PREFIX+name, desc);
	//
	// dup();
	// visitLdcInsn(true);
	// super.visitFieldInsn(Opcodes.PUTFIELD, owner,
	// Constants.BEEN_CLONED_PREFIX+name, Type.BOOLEAN_TYPE.getDescriptor());
	//
	// visitLabel(lblbForReadThrough);
	//
	// super.visitFieldInsn(opcode, owner, name, desc);
	// }
	// else if(opcode == PUTFIELD && desc.length() == 1) //If we are going to do
	// a putfield on a primitive do a simple copy
	// {
	// variablesToClear.put(name,desc);
	// super.visitFieldInsn(opcode, owner, name, desc);
	// }
	// else if(opcode == PUTFIELD && desc.length() >1) //Need a copy on a
